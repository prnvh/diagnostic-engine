...
- Fixed the root deployment miss by tracking `lib/config.js` and `lib/runtime.js` in git; they existed locally but were being excluded by the inherited `lib/` ignore rule, which left Vercel without the runtime bootstrap files.
